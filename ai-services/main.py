...
async def summarize_report(file: UploadFile = File(...)):
    ...
    try:
        ...
        raise HTTPException(status_code=422,
            detail="Could not extract text: " + str(e))
    if not extracted_text.strip():
        raise HTTPException(status_code=422,
...
async def voice_command(request: VoiceCommandRequest):
    ...
    return {"command": request.command, "intent": intent,
            "status": "processed"}
...
async def predictive_analytics():
    return {
        ...
        "readmission_risk": [
            {"patient_id":"P-1042","risk_score":0.87,"reason":"Cardiac history"},
            {"patient_id":"P-0391","risk_score":0.72,"reason":"Post-surgery"},
        ],
        "status": "success"
    }
